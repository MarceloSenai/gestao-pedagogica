...
export default function PrediosPage() {
  ...
  const handleDelete = async (id: string) => {
    ...
    const ok = await confirm({ message: `Excluir prédio "${item?.nome ?? id}"?`, confirmLabel: "Excluir" });
    if (!ok) return;
    try {
      const res = await fetch(`/api/predios/${id}`, { method: "DELETE" });
      if (!res.ok) throw new Error("Erro ao excluir");
      await fetchData();
      toast("Prédio excluído com sucesso", "success");
    } catch (err) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setModalOpen(false);
      await fetchData();
      toast("Prédio salvo com sucesso", "success");
    } catch (err) {
